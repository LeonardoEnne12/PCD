...
public class trabalho3 implements Runnable {
  ...
  public trabalho3(int[][] grid, int[][] newgrid,int n, int inicio, int fim){
    this.grid = grid;
  ...
  public void run(){
    ...
    for(i=inicio;i<=fim;i++){
      for (j = 0; j < TAM; j++)
      {
          ...
          else if(aux == 3 && grid[i][j] == 0) newgrid[i][j]=1;
          else newgrid[i][j]=0;
      }
    }
  }
  ...
  public static void main(String args[]) {
    int TAM = 2048, iteracoes = 2000, aux = 0,NTHREADS = 1, i=0,j=0,div,count=0;
    int k;
    ...
    for(i = 0; i < iteracoes; i++){
      trabalho3[] arraytrabalho = new trabalho3[NTHREADS];
      Thread[] arraythread = new Thread[NTHREADS];
      div = TAM/NTHREADS ;

      for(j = 0;j<NTHREADS;j++){
        arraytrabalho[j] = new trabalho3(grid,newgrid,TAM,aux,aux+div-1);
